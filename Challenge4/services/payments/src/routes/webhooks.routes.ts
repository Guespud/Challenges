...
import { AppError, sharedContent } from '@vitalis/shared';
...
export async function webhooksRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/webhooks/stripe', async (request, reply) => {
    ...
    if (!signature || typeof signature !== 'string') {
      throw new AppError(400, sharedContent.errors.invalidStripeSignature);
    }
    ...
    // antes (idempotencia) o si el tipo de evento no nos interesa — así
    // Stripe no lo vuelve a reintentar.
