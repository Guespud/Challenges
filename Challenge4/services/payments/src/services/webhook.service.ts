 ...
 * clave de idempotencia es `event.id`, insertado en `WebhookEvent` en su
 * propia transacción antes de aplicar cualquier efecto. Si el INSERT choca
 * con el constraint único, el evento ya se procesó — se responde 200 sin
 * reaplicar nada.
 */
export async function processCheckoutCompleted(event: Stripe.Event, requestId: string): Promise<void> {
  ...
  if (!appointmentId || !paymentIntentId) {
    logger.warn(
      { request_id: requestId, eventId: event.id },
      'checkout.session.completed sin appointmentId/payment_intent en metadata',
    );
    return;
  }
