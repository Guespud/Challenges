      ...
      <div className="pointer-events-none fixed top-4 right-4 left-4 z-50 flex flex-col items-end gap-2 sm:left-auto">
        {toasts.map((t) => (
          <div
            key={t.id}
            role="alert"
            className={`pointer-events-auto flex w-full max-w-sm items-start gap-2 rounded-2xl border px-4 py-3 text-sm font-medium shadow-[0_20px_60px_-15px_rgba(0,0,0,0.25)] ${VARIANT_STYLES[t.variant]}`}
          >
